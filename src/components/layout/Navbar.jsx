        ...
        <nav className="sticky top-0 z-[1000] h-20 bg-white shadow-soft">
            <div className="h-full px-4 md:px-8 flex items-center justify-between">
                ...
                <div className="lg:hidden flex items-center gap-2">
                    <Link to="/appointments" className="relative text-night-bordeaux p-2">
                        <svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                            <rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect>
                            <line x1="16" y1="2" x2="16" y2="6"></line>
                            <line x1="8" y1="2" x2="8" y2="6"></line>
                        ...
                        {upcomingCount > 0 && (
                            <span className="absolute top-1 right-1 min-w-[15px] h-[15px] bg-blush-rose text-white text-[8px] font-bold flex items-center justify-center rounded-full border border-white">
                                {upcomingCount}
                            </span>
                        )}
                    </Link>
                    <Link to="/cart" className="relative text-night-bordeaux p-2">
                        <svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                            <circle cx="9" cy="21" r="1"></circle>
                            <circle cx="20" cy="21" r="1"></circle>
                            <path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"></path>
                    ...
                    <button
                        onClick={toggleMenu}
                        className="text-night-bordeaux p-2 focus:outline-none"
            ...
            {/* Mobile Menu Overlay */}
            <div className={`fixed inset-0 top-20 bg-white/95 backdrop-blur-xl z-[999] lg:hidden transition-all duration-300 ease-in-out ${isOpen ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-full pointer-events-none'}`}>
                <div className="flex flex-col items-center justify-center h-full gap-8 p-8 overflow-y-auto">
                    <ul className="flex flex-col items-center gap-6">
                        <li><NavLink to="/" onClick={() => setIsOpen(false)} className="text-2xl font-heading font-black text-night-bordeaux hover:text-blush-rose transition-colors">Home</NavLink></li>
                        <li><NavLink to="/products" onClick={() => setIsOpen(false)} className="text-2xl font-heading font-black text-night-bordeaux hover:text-blush-rose transition-colors">Products</NavLink></li>
                        <li><NavLink to="/services" onClick={() => setIsOpen(false)} className="text-2xl font-heading font-black text-night-bordeaux hover:text-blush-rose transition-colors">Services</NavLink></li>
                        <li><NavLink to="/professionals" onClick={() => setIsOpen(false)} className="text-2xl font-heading font-black text-night-bordeaux hover:text-blush-rose transition-colors">Professionals</NavLink></li>
                    </ul>

                    <div className="w-full h-px bg-soft-apricot/30 max-w-[200px]" />

                    {user ? (
                        <div className="flex flex-col items-center gap-6 w-full max-w-[280px]">
                            {user.isAdmin && (
                                <NavLink to="/admin" onClick={() => setIsOpen(false)} className="text-lg font-bold text-blush-rose">Admin Panel</NavLink>
                            )}
                            <NavLink to="/dashboard" onClick={() => setIsOpen(false)} className="text-lg font-bold text-night-bordeaux">My Dashboard</NavLink>
                            <button
                                onClick={() => { logout(); setIsOpen(false); }}
                                className="w-full py-4 bg-berry-crush/10 text-berry-crush rounded-full font-black uppercase tracking-widest text-xs transition-all active:scale-95"
                            >
                                Sign Out
                            </button>
                        </div>
                        ...
                        <div className="flex flex-col items-center gap-4 w-full max-w-[280px]">
                            ...
                            <Link
                                to="/login"
                                onClick={() => setIsOpen(false)}
                                className="w-full py-4 text-center bg-white text-night-bordeaux border-2 border-night-bordeaux/10 rounded-full font-bold text-lg shadow-md transition-all active:scale-95"
                            >
                                Login
                            </Link>
                        </div>
